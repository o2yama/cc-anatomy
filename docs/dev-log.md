...
### 実機検証（2026-08-22 10:26〜10:50、開発ビルド）

`accounts.json` の `usage_cache.fetched_at` を5秒間隔で監視して挙動を観測した。

| 検証項目 | 結果 |
|---|---|
| 非ライブの照会間隔600秒（B-2） | **確認**。ライブ share3 は60秒ごと、非ライブ share1/share2 は 10:38:45 → 10:48:45 でちょうど600秒後に再照会 |
| 429 でバックオフに入る（B-3） | **確認**。429 誘発後の 10:32:45〜10:37:45 と 10:40:45〜10:45:45、いずれもちょうど5分間まったく更新されなかった（2回再現） |
| バックオフ明けの自動復帰 | **確認**。10:38:45 と 10:46:45 に復帰 |
| 429 で `claude -p` の裏起動が発火しない（A） | **確認**。バックオフ中、`ps -Ao pid=,ppid=` で cc-dashboard の子プロセスがゼロ |
| 429 時のトレイ文言 | **未確認**（表示の目視が必要） |

副産物として、`/api/oauth/usage` のレート制限は**バースト5リクエスト程度**で 429 に落ちることが分かった（連続リクエストの3〜5回目で切り替わるのを2回観測）。修正前の約4リクエスト/分を12日連続で出していれば張り付くのは必然だった。

## v0.5.3 配信後の追加修正（2026-08-22 午後、v0.5.4）

v0.5.3 を配信した直後、ユーザーから「少し時間が経つと『取得が一時的に制限されています』が必ず出てくる。ただし普通に使えてはいる」という報告を受けた。

### 何が起きていたか

v0.5.3 で入れた段階的バックオフ（5→10→20→40→60分）が**ほぼ常時オン**になっていた。バックオフ中は `live_error` が `RateLimited` 固定になるため注記が出続ける一方、数値は監視用長期トークン（`/v1/messages`）へフォールバックして毎分更新されていた。**表示中の値は新鮮なのに「最新でない可能性」と言う**、事実と食い違う状態。

原因は注記の出し分けを「ライブ直叩きが失敗したか」で判定していたこと。文言は「表示中の数値が古いかもしれない」と言っており、別ソースが新鮮な値を取れた場合にこの2つが食い違う。バックエンドは `stale` / `fetched_at` を返しているのに表示側が見ていなかった。

### 429 について実測で分かったこと

- 429 は**リクエスト頻度**によるもので、そのアカウントの使用量の枠とは**無関係**。5h 枠を 100% 使い切ったアカウント2件が 200 を返す一方、枠に余裕のある（16%）アカウントが 429 を返した。違いは「そのアカウントがライブで、アプリがポーリングしていたかどうか」
- 枠は**アカウント単位**。非ライブのアカウントはアプリがこのエンドポイントを叩かないため（監視トークン経由が最優先）、外部から連続で叩いても影響がない
- 枠は狭い。8秒間隔で3回叩けば 429 になる

途中「アカウントが使用量上限に張り付いているのが原因」という仮説を立てたが、上記の比較で**否定**した。「claude セッションが枠を共有している」という仮説も裏付けが取れていない。

### 実装した修正

| 決定 | 理由 |
|---|---|
| ライブの照会間隔を 45秒 → 300秒（5分） | 枠が狭くアカウント単位である以上、毎分1回では余裕がゼロ。同一アカウントに消費者がもう1つあれば即あふれる |
| 段階的バックオフを撤去 | 5分間隔そのものが十分なスロットル。429 が常態の環境では「実質永久に止まる」だけで、注記が出っぱなしになる害しか残らない |
| 「最後に試行した時刻」のゲートを追加（成否にかかわらず記録） | 従来のスロットルは成功時の `fetched_at` だけに依存しており、**429 が続く間は60秒ごとに叩き続けていた**。いちばん空けたい場面でスロットルが消えていた。`fetched_at` を失敗時に更新して代用してはならない（注記の古さ判定が壊れる） |
| 手動経路の下限は45秒 | ユーザー操作を5分止めるのは行き過ぎ。60秒にすると `REFRESH_INTERVAL` と同値になり到達時刻の揺れで通ったり弾かれたりするため45秒 |
| 注記の判定を `live_error` から「表示中の値の古さ」（`now - fetched_at > 600秒`）に変更 | 別ソースが新鮮な値を取れているのに「最新でない可能性」と言うのを止める。文言は `HH:MM 時点` のみ。レート制限や通信不能は対処のしようがないので原因を出さない。`Expired` のときだけ復帰案内を残す |
| ゲートで塞いだときは「直近の実際の失敗理由」を返す | 「ゲートに塞がれた」ことと「なぜ取得できなかったか」は別物。前者を後者として表示すると、HTTP を1回も打っていないのに「取得が一時的に制限されています」と言う、あるいはログイン済みが型レベルで確定している分岐で「Claude Code でログインしてください」と言うことになる |
| tray のフォールバック経路にも保持値を持たせ、ゲートを資格情報チェックの後ろへ移した | この経路にはキャッシュが無く、ゲートで塞ぐとエラー画面に落ちていた。またゲートが資格情報チェックより手前にあり、未ログインでも HTTP を打たずにレート制限を名乗っていた |

### 意図的に残した限界

- 外部で `claude auth login` して未登録アカウントに変わった場合、最大300秒だけ前アカウントの数値が出る（tray フォールバックの保持値をアカウント名前空間で分けていない）
- 失敗が続く状況では、5分ごとに1サイクルだけエラー表示にフラップする（`Failed` は保持値へフォールバックしない）
- `/v1/messages`（監視トークン経由）は使用量を消費するが、今回は触っていない。ここをどうするかは未決定

### プロセス上の記録

レビューを4巡（round4〜7）行い、**そのうち3巡で私自身が作り込んだ退行が見つかった**。特に、429 対応の途中で「HTTP を打っていないのにレート制限を名乗る」「ログイン済みなのにログインを促す」という、まさに今回直している種類の嘘を新たに2つ作っていた。実装者が書いたコメントにも事実誤認があり（実測の解釈違い）、オーケストレーター側で書き直した。また、追加されたテストの1つが本番 API を実際に叩き、ログイン済み環境ではアサーションが1つも走らない空テストになっていた（削除済み）。
