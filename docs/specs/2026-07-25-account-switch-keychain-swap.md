...
## トレイ UI の方針転換（2026-07-26）

同日中に以下の順で方針転換した。**ユーザーの好みで揺れた経緯であり、どちらが「正しい」設計と
いう話ではない**ことを明記しておく。

1. 当初、ネイティブメニュー（行を積んだ NSMenu）で「ログイン中: <表示名>」+ 5h/週次ゲージ +
   「その他のアカウント」（クリックで切り替え）+ アプリを開く/ステータス更新/バージョン確認/
   終了、を実装した（`cc5e409` → `c7c7cc2`）。下部の操作項目（ステータス更新・開く・バージョン
   確認・終了の4項目）はその後のデザイン変更（ドットバー化・ライト/ダーク対応）を経ても構成は
   変わっていない（`0be6342` で「ステータス更新」「終了」が誤って削除された時期があったが、
   2026-07-31 の最終修正ラウンドで復元済み。ラベルのみ一部改称: 「バージョン確認」→
   「バージョンを確認する」、「アプリを開く」→「CC Anatomy を開く」）。
2. 「ネイティブメニューでは見づらい」というユーザー要望を受け、CleanMyMac 風のカスタムパネル
   （フレームレス小窓、`tray-panel` ウィンドウ + `tauri-plugin-positioner` + `src/TrayPanel.tsx`）
   に全面置き換えた（`67cf3ad`）。
3. パネル化の直後に機能バグ（capabilities 未設定・`macos-private-api` 未有効による透過/invoke
   不具合）を修正しつつ、パネルの高さ可変化（`resize_tray_panel`）まで進めたが、**パネル UI 自体
   が不評**という理由で、**カスタムパネルを完全に撤回し、ネイティブメニュー（`c7c7cc2` 時点の
   実装）へ回帰した**。tauri.conf.json の `tray-panel` ウィンドウ定義、`tauri-plugin-positioner`
   依存、`macOSPrivateApi`、`src/TrayPanel.tsx`、パネル用 CSS、capabilities の `tray-panel` は
   すべて削除済み。ネイティブメニュー回帰後も、パネル化の過程で入った改善（`UsageFetch` の
   期限切れハンドリング、`switch_from_tray` の `std::thread::spawn` 分離、`info_dialog` による
   通知、表示名対応）はそのまま活かしている。

以後、トレイ UI を再検討する場合はこの経緯（2回のユーザー評価で「メニューが見づらい」→
「パネルも合わなかった」）を踏まえること。

2026-07-31: エンドユーザーの「前のデザインの方がスタイリッシュ」という声を起点にトレイUIを
再検討。カスタムパネル復活案（`feature/tray-panel-v2`、未マージ保留）と過去スタイル復元を
検討したが、最終的に新デザイン「モノクロの細いドットバー」で確定した（v0.4.6）。色閾値
（緑黄赤）は廃止し、使用分=白・残り=グレーの明度差のみで表現する。メニューバー本体の表示と
メニュー機能は不変。

2026-07-31（同日中、デザイン確定版。`feature/tray-dot-gauge`）: 上記ドットバー化の初期実装に
対し、次の3点を確定仕様として反映した。

1. **ドット間隔を詰める（案C）**: `render_dots_pixels` の `DOT_COUNT` を 20→32 に変更（キャンバス
   220×18px・直径6pxは維持。ピッチは 220/32≒6.9px、隙間は約0.9px@2x）。
2. **ライブアカウント行を1行化**: 「ラベル行＋バー画像行」の2段構成をやめ、`IconMenuItem` 1行
   （icon=使用率反映のドットバー画像、テキスト=`5H 42%（14:00 復活）` / `週次 67%（8/2 14:00
   復活）`）に統合した。アイコンは項目の先頭に付くため、2行のバーの左右端が自動的に揃う。
3. **その他アカウントはドットバー廃止 → 部分色付きテキスト行**: `5h: 12%（14:00 復活） / 週次:
   8%（8/2 14:00 復活）` の1行で、`5h: 12%` `週次: 8%` の数値部分は白 (`0xffffff`)、
   `（…復活）` とセパレータ ` / ` はグレー (`0x8e8e93`) にする。ネイティブメニュー項目は
   部分的な文字色指定ができないため、この行だけ CoreText でオフスクリーン描画した RGBA 画像
   （transparent 背景、@2x、`tray.rs` の `coretext_line` サブモジュール）を `IconMenuItem` の
   icon として渡す（テキスト自体は空）。フォントは `CTFontCreateUIFontForLanguage` で取得した
   システム UI フォント（論理13pt相当）を使い、日本語（週次・復活）はシステムフォント＋
   `CTLine` 描画によるフォントフォールバックに任せる。この行は enabled=false のクリック不可
   情報項目のまま。
   - 依存として `core-text` `core-graphics` `core-foundation` を macOS ターゲット限定
     （`[target.'cfg(target_os = "macos")'.dependencies]`）で追加。`core-graphics` は
     tauri の依存ツリーに既にある 0.25 系に固定してバージョン競合を避けた。
   - macOS 以外のビルドでは CoreText が使えないため、色を諦めて全セグメントを連結した
     プレーンテキスト1行にフォールバックする（`compact_usage_plain_text`）。
4. **切替項目のラベル変更・セクション見出し廃止**: 「その他のアカウント」という区切りの
   `MenuItem` 見出し（`info-others-header`）を廃止し、各アカウントの区切りは separator のみに
   した。切替項目のラベルも `「{display_name}」に切り替え` から、汎用の `⇄ このアカウントへ
   切り替え`（アカウント名は直前の名前行にすでに出ているため重複表示しない）に変更した。

### 2026-07-31（同日中、最終修正ラウンド）

リリース（v0.4.6）前の最終レビューで確定した3点を追加で反映した。

- **ライト/ダーク外観対応**: それまでドットバー・強調テキストの色を白固定していたため、
  macOS がライトモードのときメニュー背景（ほぼ白）に使用済み分の白ドットが溶け込み、
  「使用量が多いほど見えなくなる」逆転現象が起きていた。`build_menu` の呼び出しごとに
  `NSApplication.effectiveAppearance` を `bestMatchFromAppearancesWithNames`（aqua /
  darkAqua）で判定し、ダーク配色（既存値のまま: fill/テキスト強調=白、track/グレー=
  `0x3a3a3c`/`0x8e8e93`）とライト配色（新設: fill/テキスト強調=`0x33_33_36`、track=
  `0xd1d1d6`、テキストグレー=`0x6e6e73`）を切り替える。`MainThreadMarker` が取れない
  （メインスレッド前提が崩れている）異常系はダーク扱いにフォールバックする。判定は
  `objc2-app-kit`（tauri の依存ツリーに既にある 0.3.2 系）を使用。
- **メニュー操作項目は main の構成を維持**: `0be6342` で「ステータス更新」項目とその
  ハンドラ、「アプリを終了」項目と `"quit" => app.exit(0)` が仕様外に削除されていたのを
  復元した。下部の操作セクションは「ステータス更新」「CC Anatomy を開く」
  「バージョンを確認する」「アプリを終了」の4項目（main の並び・文言に準拠。
  「バージョンを確認する」は main の「バージョン確認」からの改称のまま据え置き）。
- **ドット数の丸め補正**: `render_dots_pixels` の四捨五入だけだと 1% が0個・99%が満杯
  （=100%と見分けがつかない）表示になっていたため、0%・100%以外は最低1個・最大
  `DOT_COUNT - 1` 個にクランプするよう補正した。

### 既知の制限

- その他アカウントの使用率行（`5h: 12%（…復活） / 週次: 8%（…復活）`）は CoreText で
  オフスクリーン描画した RGBA 画像を `IconMenuItem` の icon として渡す実装のため、
  テキストとして意味を持たない。VoiceOver 等のスクリーンリーダーでは読み上げられない
  （macOS のみ。他 OS はプレーンテキストにフォールバックするため対象外）。

## 監視用長期トークンの部分復活（2026-07-26、全廃決定の巻き戻し）

2026-07-25 に全廃した監視用長期トークン（`claude setup-token` 発行、`CC Anatomy-token-<name>`）を、
「全アカウントを常時監視したい」というユーザー要望を受けて**任意機能として復活**させた。
これは全廃決定の**部分的な巻き戻し**であり、位置づけを変えている:

- **完全に任意**。以下の統合フロー、または登録済みアカウント行の「常時監視を設定」からのみ発行
  される。ユーザーが setup-token をスキップ・キャンセルしても、アカウント追加・切り替え機能には
  一切影響しない（`accounts.rs` の setup-token スクリプトは失敗時もすべて `exit 0` で抜ける）
- **切り替え機能とは完全に独立**。監視トークンの有無・失効は `switch_account` の判断に一切絡まない
  （Keychain スワップは `CC Anatomy-cred-<name>` スナップショットのみを見る）
- 旧実装が持っていた「選択中」ポインタ `CC Anatomy-active` は復活させない
  （`meta.active` は Keychain の裏付けを持たない表示専用のブックキーピングのまま）
- 旧実装は起動時に `CC Anatomy-token-*` を一律削除するマイグレーション（`remove_legacy_monitor_tokens`）
  を持っていたが、復活に伴い削除した（これ以上消さない）

### 統合フロー（ユーザー承認済みの UX）

「＋ アカウントを追加」を、ログイン（Flow B）と監視の承認（setup-token）の2ステップに統合した:

1. **1/2 ブラウザでログイン**: 従来どおり `claude auth login` を Terminal で起動し、ライブ資格情報
   のハッシュ変化で完了検知して `import_live_account()` する。
2. **2/2 使用量監視の承認（任意）**: 1 の完了直後、**同じ Terminal スクリプト内で続けて**
   `claude setup-token` を実行する。この時点ではまだ対象アカウント名が確定していない
   （ログイン完了で確定する）ため、取得したトークンは固定の一時置き場
   `CC Anatomy-token-pending`（`PENDING_MONITOR_TOKEN_SVC`）へいったん書かせ、フロントが
   `poll_monitor_setup(name)` で対象アカウント名に claim する（コピー→pending 側を削除）。
   setup-token 側が失敗・タイムアウト（90秒）・キャンセルされても、アカウント追加自体は成功
   扱いのまま変わらない。「後から『常時監視を設定』で追加できます」と案内する。

登録済みアカウントには、行ごとに「常時監視中」バッジ、または未設定なら「常時監視を設定」の
テキストリンクを出す（setup-token のみを実行する単独フロー。同じ pending → claim の経路を使う）。

### 使用量取得の優先順位（`accounts::get_accounts_usage`、`resolve_usage_source`）

1. ライブアカウント → ライブ OAuth `/api/oauth/usage`
2. 監視用長期トークンが紐づいていれば → `/v1/messages` のレスポンスヘッダ
   （`anthropic-ratelimit-unified-*`。長期トークンは `oauth/usage` のスコープ外で拒否されるため、
   この経路でしか取れない。`actions::usage_via_monitor_token`）
3. スナップショット access token が期限内なら → それで `/api/oauth/usage`
4. どれも使えなければ → `usage_cache`（stale 表示）にフォールバック

取得結果はすべて `usage_cache` に保存し、トレイのネイティブメニュー・アカウント画面の両方が
同じキャッシュを参照する（トレイ側のコード変更は不要だった）。
